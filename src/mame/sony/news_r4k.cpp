...
protected:
	...
	void irq_w0(int state) { generic_irq_w(0, Number, state); }
	template <uint32_t Number>
	void irq_w1(int state) { generic_irq_w(1, Number, state); }
	template <uint32_t Number>
	void irq_w2(int state) { generic_irq_w(2, Number, state); }
	template <uint32_t Number>
	void irq_w3(int state) { generic_irq_w(4, Number, state); }
	void int_check();
...
void news_r4k_state::machine_common(machine_config &config)
{
	...
	m_escc->out_int_callback().set(FUNC(news_r4k_state::irq_w1<ESCC>));
	...
	m_sonic3->irq_out().set(FUNC(news_r4k_state::irq_w0<irq0_number::SONIC>));
	...
	m_hid->irq_out<news_hid_hle_device::KEYBOARD>().set(FUNC(news_r4k_state::irq_w1<KBD>));
	m_hid->irq_out<news_hid_hle_device::MOUSE>().set(FUNC(news_r4k_state::irq_w1<KBD>));
	...
	m_fdc->intrq_wr_callback().set(FUNC(news_r4k_state::irq_w0<irq0_number::FDC>));
	...
	m_dmac->irq_out().set(FUNC(news_r4k_state::irq_w0<DMAC>));
